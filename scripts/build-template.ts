...
const SOURCE = "Special Ed Scheduling.xlsx";
...
for (const sheet of [...workbook.worksheets]) {
  if (!isGeneratedSheet(sheet.name)) continue;
...
// 2a. "Service Possible" on each class sheet had no validation at all, so it
// arrived in Google Sheets as bare TRUE/FALSE text.
for (const sheetName of Object.values(input.classSheetNames)) {
  const sheet = workbook.getWorksheet(sheetName);
  if (!sheet) continue;

  const column = findColumn(
    headerColumns(sheet, 1),
    ["Service Possible", "Can Pull", "Pull OK"],
    5,
  );
  setColumnValidation(sheet, column, 2, sheet.rowCount + SPARE_ROWS, {
    type: "list",
    allowBlank: true,
    formulae: ['"TRUE,FALSE"'],
  ...
  fixes.push(`${sheet.name}: Service Possible dropdown on column ${column}`);
}
